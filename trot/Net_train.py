...
if __name__ == "__main__":
    ...
    with open('trot/dataset/trot_data_V_07_04_100000_100.pkl', 'rb') as f:
    ...
    file_path = f"result/loss/tort_loss_V_{NOWTIME}.png"
    directory = os.path.dirname(file_path)
    if not os.path.exists(directory):
        os.makedirs(directory)
    plt.savefig(file_path, dpi=300)
    plt.show()
    file_path = f"pretrain_model/tort_model_{NOWTIME}.pkl"
    directory = os.path.dirname(file_path)
    if not os.path.exists(directory):
        os.makedirs(directory)
