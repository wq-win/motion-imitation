...
def trajactory_ploter(start, end, color_array=None, x=0, y=1, z=2, u=0, v=1, w=2):
    ...
    ax.set_title(f'12 dimension, displacement * TIMESTEP * DISPLACEMENT_RATE{DISPLACEMENT_RATE}, x={x}, y={y}, z={z}')
    set_axes_equal(ax)
    plt.show()
...
def main():
    ...
    while total_times: 
        ...
        i += 1
        total_times -= 1
        print(total_times,end='\r')
    env.close()
    color = np.ones((TOTAL_TIMES, 3))
    color[:, 0] = np.linspace(0.8, 0, TOTAL_TIMES)
    color[:, 1] = np.linspace(0.8, 0, TOTAL_TIMES)
