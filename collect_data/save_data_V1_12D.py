...
# color_list.append(np.ones((p_motor_angle_tile.shape[0], 3)))
...
def calculate_point_normal_direction(point, mass_weight):
    ...
    nearst_index = np.argmin(distances)
    if distances[nearst_index] < p_motor_angle_v_rate[nearst_index]*TIMESTEP*2:
        tangent_projection = (np.dot(displacement[nearst_index], p_motor_angle_v[nearst_index])/
                              np.linalg.norm(p_motor_angle_v[nearst_index]))
        normal_vector = displacement[nearst_index] - tangent_projection*p_motor_angle_v[nearst_index]/np.linalg.norm(p_motor_angle_v[nearst_index])
        testa = np.dot(normal_vector, p_motor_angle_v[nearst_index])
...
def calculate_point_tangent_velocity(distances, decay=10):  #200
...
def repulse(direction, distances, mass_weight):
    speed = 1 / np.sum(1 / distances*mass_weight) / distances.shape[0] * CONSTAN_FACTOR
...
if __name__ == '__main__':
    ...
    #         n_direction, distances = calculate_point_normal_direction(point, mass_weight)
    #         n_displacement = repulse(n_direction, distances, mass_weight)
    #         t_displacement = calculate_point_tangent_velocity(distances)
    #         displacement = t_displacement + n_displacement # + t_displacement
    #         point += displacement
    ...
    trajactory_ploter(input_array, output_array, index_range=[0000, 1000], dim=num_joints, color_array=color_array,  x=0, y=1, z=2, u=0, v=1, w=2)
    trajactory_ploter(input_array, output_array, index_range=[0000, 1000], dim=num_joints, color_array=color_array, x=3,
                      y=4, z=5, u=3, v=4, w=5)
    trajactory_ploter(input_array, output_array, index_range=[0000, 1000], dim=num_joints, color_array=color_array, x=6,
                      y=7, z=8, u=6, v=7, w=8)
    trajactory_ploter(input_array, output_array, index_range=[0, 1000], dim=num_joints, color_array=color_array, x=9,
