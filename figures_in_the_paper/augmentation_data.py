...
def calculate_point_normal_direction(ma_array, point, if_next=True):
    ma_v, ma_v_norm, ma_weight, point2ring_displacement, point2ring_nearest_displacement, point2ring_displacement_norm, point2ring_nearest_index, ring_nearest_index_v, ring_nearest_index_v_norm, distances_flag = new_func(ma_array, point, if_next=if_next)
    if distances_flag:
...
def new_func(ma_array, point, if_next=True):
...
def repulse(normal_direction, ma_array, point):
    ...
    speed = 0.3 * np.min(point2ring_displacement_norm)
...
if __name__ == '__main__':
    ...
    for _ in tqdm.tqdm(range(SAMPLE_POINT_NUMS // 2)):
        ...
        for i in range(ITER_TIMES):
            ...
            normal_direction = calculate_point_normal_direction(tma, point, if_next=False)
