...
def new_func(ma_array, point):
    ...
    if np.dot(point2ring_displacement[point2ring_nearest_index_temp],
        ...
        point2ring_nearest_index %= PACE_LEN
    else:
...
if __name__ == '__main__':
    ...
    for _ in tqdm.tqdm(range(SAMPLE_POINT_NUMS // 2)):
        point = sample_random_point(oma)
        for i in range(ITER_TIMES):
            input_list.append(copy.deepcopy(point))
            normal_direction = calculate_point_normal_direction(oma, point)
            normal_displacement = repulse(normal_direction, oma, point)
            tangent_displacement = calculate_point_tangent_velocity(oma, point)
            displacement =  tangent_displacement + normal_displacement 
            displacement = calculate_point_displacement(oma, point, displacement)
    ...
    file_path = f'collect_dataset/oma_data_V_{NOWTIME}_{SAMPLE_POINT_NUMS}_{ITER_TIMES}.pkl'
